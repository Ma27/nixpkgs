...

{
  ...
  config = mkMerge [
    ...
    (mkIf config.boot.kernel.enable {
      ...
      lib.kernelConfig =
        if config.boot.kernelPackages.kernel ? buildtimeConfig then
          config.boot.kernelPackages.kernel.buildtimeConfig
        else
          {
            isYes = option: {
      ...
      system.checks = [
        (config.boot.kernelPackages.kernel.buildtimeConfig.mkAssertions config.boot.kernelPackages.kernel.configfile config.system.requiredKernelConfig)
      ];
